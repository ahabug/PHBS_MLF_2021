...
def plot_most_common_words(df, article_num=9):
    ...
    sns.barplot(x=[x[0] for x in df.iloc[article_num]['common_20_stopped_lemm_words']],
                y=[x[1] for x in df.iloc[article_num]['common_20_stopped_lemm_words']], ax=left,
                color='#ffd966')  # palette = mycmap)
    ...
    sns.barplot(x=[x[0] for x in df.iloc[article_num]['common_20_lemm_words']],
                y=[x[1] for x in df.iloc[article_num]['common_20_lemm_words']], ax=right,
                color='gray')  # palette = mycmap)
...
def run_and_save_final_lda_model(bow_corpus,
                                 dictionary,
                                 df,
                                 col_name,
                                 num_topics=11,
